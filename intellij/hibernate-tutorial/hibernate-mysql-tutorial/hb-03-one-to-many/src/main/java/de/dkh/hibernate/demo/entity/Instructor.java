 ...
 * NOTE: the {@linkplain FetchType#EAGER} is selected here,
 * because we want to read courses from instructor after we get him from db.
 */
@Entity
...
public class Instructor extends PersistentObject implements Serializable {
    ...
    @OneToMany(fetch = FetchType.EAGER, mappedBy = "instructor", cascade = {CascadeType.MERGE
            , CascadeType.DETACH
            , CascadeType.PERSIST
            , CascadeType.REFRESH})
    @Getter
